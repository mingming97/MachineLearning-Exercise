...
Matrix::Matrix(int i, int j, double val = 0) {
	row = i;
	col = j;
	elem = new double[i * j];
...
Matrix::Matrix(int rows, double val = 1) {
	row = col = rows;
...
Matrix::Matrix(const vector<double> &v) {
	row = col = v.size();
	elem = new double[row * row];
	for (int i = 0; i < row; i++)
		for (int j = 0; j < row; j++)
			elem[i * col + j] = (i == j ? v[i] : 0);
}
...
void Matrix::orth(double *a, double *b, int n) {//|a|=1 令b成为a的正交向量 
